	...
	public void keyPressed(KeyEvent e) {
		...
		for(int i=0;i<handler.objects.size();i++) {
			...
			if(temObject.getID()==ID.Player) {
				...
				if(key == KeyEvent.VK_SHIFT) {temObject.velX*=3; temObject.velY*=3;}															//KeyEvents falls es ein spieler ist(Die Unterscheidung ist wichtig, falls mann mehrspieler-Modi oder verschiedene Charaktere oder so haben will)
				if(key == KeyEvent.VK_UP) temObject.shoot(Direction.Up);																		//WENN DIE TASTEN GEDRÜCKT WERDEN
	...
	public void keyReleased(KeyEvent e) {
		...
		for(int i=0;i<handler.objects.size();i++) {
			...
			if(temObject.getID()==ID.Player) {
				...
				if(key == KeyEvent.VK_SHIFT) {temObject.velX/=3; temObject.velY/=3;}	
